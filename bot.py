...
def get_level_name(total_spent: float) -> str:
...
def get_next_level_info(total_spent: float) -> str:
...
async def get_pending(pending_key: str):
...
async def delete_pending(pending_key: str):
...
async def save_redeem(data: dict):
...
async def get_redeem(user_id: int):
...
async def delete_redeem(user_id: int):
...
async def balance(
    ...
    text = (
        f"💳 *Ваш баланс*\n\n"
        f"Баллы: *{u['points']:.2f}* 🪙\n"
        f"Всего потрачено: *{u['total_spent']:.2f}₽*\n"
        f"Уровень: {get_level_name(u['total_spent'])}\n"
...
async def purchase_amount(
    ...
    keyboard = [[
        InlineKeyboardButton(
            "✅ Подтвердить",
            callback_data=f"approve_{pending_key}"
        ),
        InlineKeyboardButton(
            "❌ Отклонить",
            callback_data=f"reject_{pending_key}"
        ),
    ]]
...
async def redeem(
    ...
    if u["points"] < 1:
        ...
        await update.message.reply_text(
            "😔 У вас недостаточно баллов для обмена."
        )
    ...
    await update.message.reply_text(
        f"🎁 *Ваш код на скидку*\n\n"
        f"🪙 Баллов: *{u['points']:.2f}*\n"
        f"💰 Скидка: *{discount}₽*\n\n"
        f"Ваш код:\n`{promo_code}`\n\n"
        "Отправьте этот код в нашу группу ВК:\n"
        "https://vk.me/poputopia\n\n"
        "_После проверки скидка будет подтверждена._",
        parse_mode="Markdown"
    )
    ...
    keyboard = [[
        InlineKeyboardButton(
            "✅ Скидка выдана",
            callback_data=f"redeem_approve_{user.id}"
        ),
        InlineKeyboardButton(
            "❌ Отклонить",
            callback_data=f"redeem_reject_{user.id}"
        ),
    ]]
...
async def redeem_callback(
    ...
    if action == "approve":
        ...
        await context.bot.send_message(
            chat_id=user_id,
            text=(
                "😔 Ваш запрос на скидку был отклонён.\n\n"
                "Если вы считаете это ошибкой — "
                "свяжитесь с нами: "
                "https://vk.me/poputopia"
            ),
